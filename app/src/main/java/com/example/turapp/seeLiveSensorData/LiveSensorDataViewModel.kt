...
class LiveSensorDataViewModel(app: Application) : ViewModel() {
    ...
    private var _recOrientation : Boolean = false // Just a helper to know if we are currently recording this data
    ...
    init {
        ...
        accSensor.setOnSensorValuesChangedListener {
            ...
            // Record new value
            _accSensorData.value =
                filterOutGravity(it as MutableList<Float>, _prevAccData.value ?: mutableListOf())
            updateOrientationAngles()
        }
    ...
    private fun storeRecording(timeTaken: Long) {
        viewModelScope.launch {
            ...
            if (_orientationRec.size > 0) {
                dao.insertRecording(
                    Recording(
                        poiId = id.toInt(), sensorType = Sensor.TYPE_HEADING,
                        recording = _tempGyroSensorRec.toString()
                    )
                )
            }
